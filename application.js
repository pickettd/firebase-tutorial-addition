...
recommendations.once('value').then(function(snapshot) {
  var allRecommendations = snapshot.val();
  var htmlString = '';
  if (allRecommendations) {
    var recKeys = Object.keys(allRecommendations);
    recKeys.forEach(function(recKey) {
      var recommend = allRecommendations[recKey];
      htmlString = '<tr><td>'+recommend.title+'</td><td>'+recommend.presenter+'</td><td><a target="_blank" href="'+recommend.link+'">'+recommend.link+'</a></td></tr>';
      $( htmlString ).appendTo( "#recTableBody" );
    });
  }
});
